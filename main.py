...
print("ШОРТ-КИЛЛЕР 2025 | спайк ×3.3 — ГОТОВ К БОЮ")
...
while True:
    try:
        ...
        for symbol in [s for s in markets if s.endswith('/USDT') and markets[s]['spot']]:
            ...
            if (price >= 0.00015 and rsi >= 78 and price >= max(closes[-15:]) * 0.995 and 
                volume_spike >= 3.3 and (change_20m >= 9 or change_45m >= 14)):

                chart_buf, resistance, supports, sl = draw_chart_with_levels(ohlcv[-45:])

                tp1 = supports[0] if supports else price * 0.84
...
Фьючерсы: {platform.upper()}"""
                ...
                sent.add(symbol)
                print(f"СИГНАЛ → {base} {price:.6f} RSI {rsi:.1f} ×{volume_spike:.1f}")
                time.sleep(3)

        print(f"{datetime.now().strftime('%H:%M:%S')} — сканирую (спайк ≥3.3)")
        time.sleep(48)
